...
function patchFor(site, msg) {
  if (msg.type === "state") {
    ...
    // ============================================================
    // תיאור התקלה — שלוש התנהגויות שונות, ואסור למזג אותן
    // ============================================================
    // ⚠️ בלי הטיפול הזה תקלה שמגיעה **חיה** הופכת את הכרטיס ל"מושבת" בלי
    // שום תיאור, עד הריענון המלא הבא. כלומר דווקא ברגע שהמידע הכי דחוף
    // הוא לא שם.
    //
    //   1. **ההודעה נושאת תיאור** → מציבים אותו. זו תקלה חדשה.
    //   2. **מעבר לתחזוקה בלי תיאור** → **משאירים את הקיים.** זה הטיפול
    //      בתקלה שכרגע הוצגה, וניקויו היה מוחק בדיוק את מה שהמשתמשת
    //      ביקשה שיישאר.
    //   3. **מעבר למוכן/בפעולה** → מנקים. התקלה נגמרה, והשארת התיאור
    //      הייתה מתארת עבר על כרטיס שמתאר הווה.
    //
    // ⚠️ undefined ולא null במקרה 2: הפצה עם undefined **אינה** דורסת את
    // הערך הקיים ב-{...site, ...patch}, ואילו null היה מוחק אותו.
    const faultPatch =
      typeof msg.faultText === "string" && msg.faultText
        ? { currentFaultText: msg.faultText }
        : msg.newStatus === "maintenance"
          ? {}                              // משאירים כפי שהוא
          : msg.newStatus === "error"
            ? { currentFaultText: null }    // תקלה חדשה בלי תיאור
            : { currentFaultText: null };   // חזרה לפעילות — מנקים
