...
function ActivityLog({ log }) {
  ...
  const visible = useMemo(() => {
    ...
    // "הכל" — ציר הזמן המאוחד. שורת מצב שיש לה פעולה שמסבירה אותה מוסתרת:
    // מעבר MODE אחד מוצג כשורה אחת, לא כשתיים שנראות סותרות. מצב בלי פעולה
    // תואמת (resync, תקלה, תחזוקה, נתק) נשאר תמיד.
    if (filter === "all") return entries.filter((e) => !e.explainedByOp);
  ...
  const countFor = (key) => ({
    ...
    // תחזוקה + מקטעי 'בפעולה' **יתומים** (אלה שאין להם פעולה, ולכן אינם
    // מוסתרים). ה-?? מגן על שרת ישן שעדיין לא שולח את השדה.
    all: counts.operations + counts.status + counts.maintenance
       + (counts.orphanOperating ?? 0),
