...
const FILTERS = [
  ...
  // התחזוקה, בלי חפיפה. "טיפול בתקלה" קודם — הוא השאלה שנשאלת בפועל
  // ("כמה פעמים באו לתקן"), בעוד תחזוקה שגרתית היא הרקע.
  { key: "repair", label: "טיפול בתקלה" },
...
function describe(e) {
  if (e.kind === "operation") {
    ...
    if (rescued) details.push("הרכב הועבר ידנית בטיפול בתקלה");
  ...
  return {
    // צבע התחזוקה נשמר גם לטיפול בתקלה: הוא עדיין תחזוקה לכל חישוב, וצביעה
    ...
    // ==========================================================
    // ⚠️ קודם היה כאן "טיפול בתקלה" מול "המצב השתנה ל: בתחזוקה". שתי צורות
    // שונות לגמרי, ולכן מי שסורק את הלוג אינו רואה שתי קטגוריות של אותו
    // דבר — הוא רואה אירוע מיוחד אחד, ועוד שינוי מצב רגיל.
    //
    // עכשיו שתיהן שם עצם קצר, כמו במסך הזמינות ובתובנות: **טיפול בתקלה**
    ...
    details: [
      ...
      isRepair
        ? `החל מיד עם סיום התקלה · ${e.endedAt ? `נמשך ${dur}` : "עדיין בטיפול"}`
        : isPlcMaintenance
