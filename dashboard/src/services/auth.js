...
export async function sendMagicLink(email) {
  ...
  const { error } = await supabase.auth.signInWithOtp({
    ...
    options: {
      ...
      // ⚠️ **false, וזה שינוי מהותי.** `true` ביקש מ-GoTrue ליצור משתמש
      // לכל כתובת שהוקלדה — וזה סותר ישירות את "אין הרשמה עצמית". בפועל
      // המסד היה חוסם, אבל המשתמש היה מקבל `Unexpected failure` גנרי
      // במקום תשובה מובנת. עכשיו GoTrue אומר בעצמו שאין כזה משתמש.
      shouldCreateUser: false,
    },
  });

  if (error) {
    // ⚠️ אותה הודעה שקרית שהייתה ב-requestPasswordReset — ראה ההסבר שם.
    // "המתן דקה" מבטיחה שההמתנה תפתור, בעוד שבלי SMTP המייל לא יגיע
    // לעולם. ההודעה אומרת מה נכון ומה לעשות.
    return {
      error: /rate limit|too many/i.test(error.message)
        ? "שליחת המיילים אינה זמינה כרגע, ולכן הקישור לא נשלח. " +
          "התחברו עם סיסמה, או פנו למנהל."
        : error.message,
    };
  }
  return { error: null };
...
export async function requestPasswordReset(email) {
  ...
  if (error) {
    // ============================================================
    // ⚠️ "המתן דקה ונסה שוב" הייתה הודעה **שקרית**
    // ============================================================
    // היא מבטיחה שהמתנה תפתור, ולכן שולחת ללחוץ שוב ושוב. אבל כל עוד
    // לא הוגדר SMTP, המערכת נופלת על המיילר המובנה של Supabase — שמוגבל
    // לכמה מיילים בשעה **ושולח רק לחברי הפרויקט**. כלומר לרוב המשתמשים
    // המייל לא יגיע לעולם, בכמה שעות שימתינו.
    //
    // נמדד מול המופע: `429 over_email_send_rate_limit` על הבקשה הראשונה
    // באותה שעה.
    //
    // ⚠️ ולכן ההודעה אומרת מה שנכון **ומה לעשות**: בלי SMTP מסלול
    // ההתאוששות היחיד הוא מנהל שמזמין מחדש ומעביר סיסמה זמנית. משתמש
    // שממתין לשווא אינו יודע שיש דרך אחרת.
    const isRate = /rate limit|too many/i.test(error.message);
    return {
      error: isRate
        ? "שליחת המיילים אינה זמינה כרגע, ולכן קישור האיפוס לא נשלח. " +
          "פנו למנהל — הוא יכול להנפיק לכם סיסמה חדשה."
        : error.message,
    };
  }
  return { error: null };
