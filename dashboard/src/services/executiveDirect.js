...
async function runExecutive({ from, to, ...filters }) {
  ...
  const [supervisor, seriesRes, sitesRes] = await Promise.all([
    ...
    // ⚠️ הגרסה שמחזירה שורה אחת, ולא זו שמחזירה שורות — תיקון נכונות.
    // PostgREST חותך כל תשובה ב-1,000 שורות, והפונקציה מחזירה שורה לכל
    // (דלי × אתר). נמדד בייצור: "השנה הנוכחית" ברזולוציה יומית = 3,920
    // שורות, מהן חזרו 1,000 — **בסטטוס 200**, בלי שגיאה ובלי 206. גם
    // "הרבעון הנוכחי" כבר נחתך היום (1,024 → 1,000).
    //
    // ⚠️ ולפונקציה אין ORDER BY, ולכן אלה 1,000 שורות שרירותיות: הגרף לא
    // נגמר באמצע אלא מתפזר — דליים עם נתונים ודליים באפס לסירוגין, דפוס
    // שנראה בדיוק כמו מכונה שקטה. `Range` אינו עוזר; התקרה היא הגדרת שרת.
    supabase.rpc("executive_series_json", {
      p_site_ids: null,
      p_from: from,
      p_to: to,
      p_buckets: withTotal,
