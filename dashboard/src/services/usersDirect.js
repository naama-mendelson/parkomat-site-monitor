...
//
// ============================================================
// ⚠️ שלוש פעולות מתוך חמש — והשתיים שנשארו אינן יכולות לעבור
// ============================================================
// `inviteUser` ו-`deleteUser` **נשארים בשרת**, וזו לא עצלות: יצירה ומחיקה
// של משתמש ב-GoTrue עוברות ב-Admin API, כלומר דורשות את מפתח ה-Secret.
// המפתח הזה עוקף RLS לחלוטין — הכלל בשורש CLAUDE.md אוסר עליו להגיע
// לדפדפן, ובצדק: מי שמחזיק אותו יכול לקרוא ולכתוב כל שורה בכל טבלה.
//
// ⚠️ **זו התשובה המלאה ל"למה ניהול המשתמשים לא ישר ב-Supabase":** שלוש
// פעולות כן — הן נוגעות רק ב-`app_users`. שתיים לא, כי הן נוגעות ב-GoTrue.
//
// ============================================================
