 ...
 */
// ⚠️ הכתובת צרובה כברירת מחדל, וזו הפשרה מודעת. כתובת LAN בתוך
// בניית ייצור אינה יפה — אבל החלופה נמדדה והיא גרועה בהרבה: בלי ערך כאן,
// `fixflowLinkFor` מחזיר `null` וכל הפיילוט נעלם — וזה בדיוק מה שקרה.
// המשתנה גובר על הברירה, ולכן מעבר של FixFlow למכונה אחרת הוא שורה ב-.env.
//
// ⚠️ השרת רץ במשרד, ולכן כפתור "תקלות ופתרונות" עובד **רק ברשת
// המשרד**. השיוך עצמו אינו תלוי בשרת — הבורר קורא מהמפה שנארזה פנימה.
export const FIXFLOW_BASE_URL = String(
  import.meta.env.VITE_FIXFLOW_BASE_URL ?? "http://192.168.1.93:3001"
).replace(/\/+$/, "");
