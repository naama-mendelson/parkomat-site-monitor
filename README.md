...
### מעבר ממחשב פיתוח לשרת — סדר הפעולות חשוב

`restart: unless-stopped` הוא מה שהופך את השרת ל-always-on: הוא מרים אחרי קריסה,
אחרי אתחול המכונה, ובלי שאיש יהיה מחובר. **זה המסלול היחיד לפרודקשן.**

במחשב פיתוח (בלי Docker) יש חלופה זמנית — משימה מתוזמנת, ראה
[`master/tools/autostart/`](master/tools/autostart). היא אינה עוברת לשרת ואינה
אמורה לעבור.

> ⚠️ **לכבות את הישן לפני שמרימים את החדש. לא במקביל.**
>
> ```powershell
> Unregister-ScheduledTask -TaskName "Parkomat SiteMonitor Server" -Confirm:$false
> Get-Process node | Where-Object { $_.CommandLine -like "*master.js*" } | Stop-Process
> ```
>
> שני עותקים של השרת מתחברים ל-HiveMQ עם אותו `MASTER_CLIENT_ID`. תקן MQTT
> מחייב ייחודיות, ולכן **הם מנתקים זה את זה בלולאה אינסופית ואף אחד לא קולט
> כלום**. זה נצפה בלוג בפועל, והתסמין מטעה: נראה בדיוק כמו "השרת החדש לא עובד".
>
> **אין צורך למהר.** כל עוד שום עותק לא רץ, HiveMQ צובר את ההודעות
> (`clean:false` + clientId קבוע) ומוסר את כולן כשהשרת החדש יעלה — עם החותמים
> המקוריים. נמדד: 15 שעות השבתה, 240 הודעות, אפס אובדן.
...
- ⚠️ **`POST`/`DELETE /api/sites/:code/maintenance` פתוחים לכל אחד, בהחלטת מוצר
  מכוונת.** תחזוקה אינה תווית: היא מדכאת רישום תקלות לחלוטין (הודעת `error` בזמן
  תחזוקה נזרקת ואינה נרשמת ב-`status_history`, אינה נספרת ואינה מתריעה) ומוחרגת
  ממכנה הזמינות. כלומר **כל קורא HTTP יכול להשתיק אתר עד 30 יום**, והמערכת תציג
  אותו כתקין.

  הכלל הוא **ייחוס ולא מנע**, וזו הייתה הכוונה מההתחלה — כפתור התחזוקה בדשבורד
  מעולם לא היה מוגן לפי תפקיד, והטופס תמיד דרש להקליד שם. מה שמאזן:
  - **שם הוא חובה** — בלעדיו 400.
  - `set_by_name` מעדיף זהות מאומתת (אסימון) על טקסט מהגוף.
  - **שורת ביקורת בלוג** לכל הפעלה וכל ביטול, עם שם, IP, ודרגת אמון
    (`token` / `admin-code` / `anonymous`).

  **לפני חשיפה פומבית** יש להפוך את זה לחוסם: שורה אחת ב-`identifyActor`
  (`api/routes.js`) — להסיר את ההערה מה-`return res.status(401)`.
