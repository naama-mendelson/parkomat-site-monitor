...
### עותק אחד בלבד — במעבר לשרת, ובכלל

`restart: unless-stopped` הוא מה שהופך את השרת ל-always-on: הוא מרים אחרי קריסה,
אחרי אתחול המכונה, ובלי שאיש יהיה מחובר.

> ⚠️ **לוודא ששום עותק אחר אינו רץ לפני שמרימים את הקונטיינר.**
>
...
> כלום**. בלוג רואים `connected → החיבור נסגר → מתחבר מחדש` בלי סוף.
>
> זה נצפה בפועל, והתסמין מטעה לחלוטין: נראה בדיוק כמו "השרת החדש לא עובד",
> בזמן שהבעיה היא שהישן עוד חי. זו גם הסיבה ש-`docker-compose.yml` אוסר
> `replicas` / `scale`.
>
> **אין צורך למהר בין כיבוי הישן להרמת החדש.** כל עוד שום עותק לא רץ, HiveMQ
> צובר את ההודעות (`clean:false` + clientId קבוע) ומוסר את כולן כשהשרת יעלה —
> עם החותמים המקוריים. נמדד: 15 שעות השבתה, 240 הודעות, אפס אובדן.
