...
public sealed class PendingQueueTests : IDisposable
{
    ...
    [Fact]
    public void TheDefaultCapSurvivesAWeekLongOutage()
    {
        // ============================================================
        // ⚠️ המספר עצמו הוא ההתנהגות, ולכן הוא נבדק
        // ============================================================
        // באביגיל 20 (22/09/2026) חומת האש חסמה את Supabase חמישה ימים.
        // התקרה הישנה (1,000) נחצתה, ומאותו רגע כל הודעה חדשה דחפה ישנה
        // החוצה — אובדן אמיתי, בלי שום סימן חוץ מ-`(1000 waiting)` בלוג.
        //
        // בקצב של אתר טיפוסי (כ-200 הודעות ביום) 10,000 הם כשבעה שבועות.
        // ⚠️ ובדיקה על המספר ולא על "גדול מ-1000": ירידה חזרה היא בדיוק
        // סוג השינוי שנעשה בטעות ואי אפשר להבחין בו עד הנתק הבא.
        Assert.Equal(10_000, PendingQueue.DefaultMaxFiles);

        // ובלי תקרה כלל תיקייה מלאה מפילה את המחשב — הגבול עדיין נאכף.
        var q = new PendingQueue(_dir, maxFiles: 3);
        for (int i = 0; i < 6; i++) q.Enqueue(Op(2000 + i, $"u{i}"));
        Assert.True(q.LoadAll<OperationMessage>().Count <= 3);
    }
