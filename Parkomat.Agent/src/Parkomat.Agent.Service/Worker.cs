...
public class Worker : BackgroundService
{
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        const int FaultTextSettleMs = 250;
        ...
        string? ReadFaultTextOrNull(SiteState state)
        {
            ...
            // ============================================================
            // ⚠️ שתי קריאות תמיד — כי הזבל אינו תמיד נראה כמו זבל
            // ============================================================
            // הטקסט יושב בכתובת 2 וה-MODE ב-290, ולכן הם **אינם נקראים
            // באותו round-trip**. ההערה ב-PlcReader קיבלה את זה בטענה
            // שהתיאור משתנה כשהתקלה משתנה ולא בתוכה — וזו ההנחה שנשברה.
            //
            // ⚠️ נמדד באתר 1376, 20.08: הבקר עבר תחזוקה→תקלה תוך **12
            // שניות** והטקסט עוד לא נכתב. באותו יום, מעברים של 25 שניות
            // ומעלה החזירו עברית תקינה — כלומר תחרות תזמון, לא קידוד.
            //
            // ⚠️ **והגרסה שבדקה רק HadUnknown לא הספיקה.** חוצץ ישן יכול
            // להחזיק את התיאור של התקלה **הקודמת** — טקסט תקין לחלוטין,
            // שאינו מפעיל שום אזהרה, ושולח את מי שקורא אותו לחפש את
            // הבעיה הלא נכונה. זו בדיוק המלכודת ש-`珼` דווקא מנע.
            //
            // לכן: קוראים, ממתינים, קוראים שוב. אם השתיים **מסכימות**,
            // הבקר סיים לכתוב. אם לא — השנייה מאוחרת יותר ולכן נכונה יותר.
            FaultText first = plc.ReadFaultText();
            Thread.Sleep(FaultTextSettleMs);
            FaultText ft = plc.ReadFaultText();

            if (first.Text != ft.Text)
            {
                // ⚠️ זו אינה שגיאה — זו בדיוק המדידה שהתיקון קיים בשבילה.
                // היא מופיעה בלוג כדי שנדע כמה פעמים זה באמת קורה בשטח.
                _logger.LogInformation(
                    "Fault text was still settling: first read '{First}', " +
                    "after {Ms}ms '{Second}'. Using the second.",
                    first.Text, FaultTextSettleMs, ft.Text);
            }
