...
public class OperationDetector
{
    ...
    public DetectionResult Process(int mode, string cardNumber, int cycleCounter)
    {
        ...
        else if (IsOperationMode(mode)
                 && string.IsNullOrEmpty(_operationCard)
                 && !string.IsNullOrEmpty(cardNumber))
        {
            // ==========================================================
            // ממלאים רק כשאין כרטיס — לא דורסים כרטיס קיים
            // ==========================================================
            // הכוונה המקורית הייתה לטפל בבקרים שמאפסים את רגיסטר הכרטיס לפני
            // שה-MODE יוצא ממצב הפעולה. אבל התנאי היה "כל כרטיס לא-ריק", ולכן
            // הוא אימץ גם כרטיס של רכב **אחר**: נהג שמעביר כרטיס בזמן שהפעולה
            // הקודמת עוד רצה גנב אותה.
            //
            // ⚠️ זה לא נראה כנתון חסר אלא כנתון תקין, ולכן הוא לא התגלה. נמדד
            // בשרת: 86 מתוך 1,013 זוגות (8.5%) נסגרו עם כרטיס שאינו שלהם,
            // ובאתר אחד (חולדה 4) 66 מהם. הרצף בגולדברג 5 הדגים:
            //
            //     03:45  יציאה/פתיחה  כרטיס 10
            //     03:51  יציאה/סגירה  כרטיס 6    <- הכרטיס של הפעולה הבאה
            //
            // התוצאה: מאזן כניסות/יציאות שנראה בלתי אפשרי לכרטיס בודד, למשל
            // 6 כניסות מול 3 יציאות — כי כרטיס אחד "נכנס" בשם אחר.
            //
            // הכלל הנכון: הכרטיס נלכד בתחילת הפעולה ואינו מתחלף בתוכה. המילוי
            // כאן נשאר רק למקרה שהפתיחה עצמה הגיעה בלי כרטיס — הבקר עוד לא
            // קרא אותו באותה מילישנייה.
            _operationCard = cardNumber;
        }
