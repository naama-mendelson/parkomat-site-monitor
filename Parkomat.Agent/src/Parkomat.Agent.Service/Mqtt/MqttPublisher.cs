...
public class MqttPublisher : IAsyncDisposable
{
    ...
    private async Task PublishAsync(string topic, object payload, CancellationToken ct)
    {
        ...
        // ============================================================
        // ⚠️ הצופה — התפר היחיד שדרכו **כל** שידור עובר
        // ============================================================
        // Worker משדר משישה מקומות שונים, וחלקם בונים את ההודעה בתוך
        // הקריאה עצמה. שכפול הכתיבה בכל אחד מהם היה עניין של זיכרון:
        // אתר שביעי שיתווסף מחר לא יימסר לשום מקום, בלי שגיאה ובלי סימן.
        //
        // ⚠️ **וזה אינו קישור בין MQTT ל-Supabase.** המחלקה הזו אינה יודעת
        // מי מאזין ולא למה — היא מדווחת "פרסמתי X", בדיוק כמו SentAuditLog
        // שורה מעליה. Worker הוא שמחליט מה לעשות עם זה.
        //
        // ⚠️ ונקרא **אחרי** פרסום מוצלח בלבד: הודעה שנכשלה ב-MQTT אינה
        // אמורה להיכתב במקום אחר כאילו נמסרה.
