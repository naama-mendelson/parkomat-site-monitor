...
public class PlcConfig
{
    ...
    // ============================================================
    // ⚠️ מערכת שנייה באותו אתר — ריק בכל האתרים חוץ מפלורנטין
    // ============================================================
    // באתר פלורנטין יש שני ParkManager באותו בקר, כל אחד עם מצב ורכב
    // משלו (290/291 ו-293/294), ומונה מחזורים **אחד משותף** (292).
    //
    // ⚠️ **אפס = אין מערכת שנייה, וזה נגזר ולא נשמר.** אין דגל
    // "דו-מערכתי" נפרד, כי אז היה אפשר לבטא "דו-מערכתי בלי רגיסטרים"
    // — מצב שאין לו משמעות. אותו עיקרון כמו SupabaseConfig.Enabled.
    //
    // ⚠️ **ושניהם נדרשים.** מצב בלי רכב אינו חצי-תכונה אלא הגדרה
    // שבורה: היה נרשם תפעול בלי מספר כרטיס, בשקט.
    public int ModeRegister2 { get; set; } = 0;

    /// <summary>רגיסטר הרכב של המערכת השנייה. 0 = אין מערכת שנייה.</summary>
    public int CardRegister2 { get; set; } = 0;

    /// <summary>נגזר: לאתר יש שתי מערכות רק אם **שני** הרגיסטרים הוגדרו.</summary>
    public bool HasSecondSystem => ModeRegister2 > 0 && CardRegister2 > 0;
