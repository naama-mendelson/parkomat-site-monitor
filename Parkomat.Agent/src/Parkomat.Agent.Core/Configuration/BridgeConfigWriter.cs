...
public static class BridgeConfigWriter
{
    ...
    public static string Build(SiteConfig config)
    {
        ...
        // ==========================================================
        // ⚠️ cleansession **false**, ולא true — נמדד, לא הונח
        // ==========================================================
        // כאן היה `true`, והוא היה השורה היחידה בקובץ הזה בלי נימוק.
        //
        // ⚠️ **מה שהוא עשה, במדידה:** ניסוי מבוקר עם שני ברוקרים מקומיים
        // (tools/cleansession-test.sh), אותו קונפיג בדיוק, ניתוק יזום של
        // היעד, ואז חמש הודעות QoS 1 ל-Mosquitto המקומי:
        //
        //     cleansession true   →  0 מתוך 5 הגיעו
        //     cleansession false  →  5 מתוך 5 הגיעו
        //
        // כלומר כל הודעה שנוצרה באתר בזמן נתק אינטרנט **אבדה לתמיד**.
        // לא התעכבה — אבדה. עם `true` הברוקר המקומי מוחק את המנוי של הגשר
        // בכל ניתוק, ולכן אין למי לצבור.
        //
        // ⚠️ וההערה ב-Worker.cs טענה במפורש את ההפך ("Mosquitto מצבור את
        // ההודעות ומזרים אותן כשהגשר חוזר"). היא תוקנה יחד עם זה.
        //
        // ⚠️ **מה זה מחזיר ומה לא, במדויק.** ההודעות שנצברות נושאות את
        // החותם המקורי שלהן:
        //   • **תפעולים** — נקלטים. הדדופ בשרת הוא על occurred_at, וזה
        //     backfill אמיתי. זה הנתון העסקי, וזה מה שאבד עד היום.
        //   • **הודעות state מאוחרות** — עדיין נדחות בשומר ה-backfill,
        //     וזה **נכון**: ה-resync בחיבור מחדש כבר מסר את המצב הנוכחי
        //     עם חותם טרי, ומצב ישן שידרוס אותו היה מזיק.
        //
        // מה שעדיין **לא** מכוסה: נפילת חשמל. התור של Mosquitto יושב
        // בזיכרון ובדיסק של אותו מחשב, וכשהוא מת הכול מת איתו. הפתרון לזה
        // הוא תור עמיד בסוכן עצמו, וזו עבודה נפרדת.
        sb.AppendLine("cleansession false");
        sb.AppendLine();
