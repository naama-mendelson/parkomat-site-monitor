...
export function computeInsights({ ops: opsIn, errorRows, maintRows, windows, from, to, siteNames, allRows }) {
  ...
  // ============================================================
  // ⚠️ רצפת סבירות — 0.2% מהנתונים קבעו 100% מ"הפעולה הקצרה ביותר"
  // ============================================================
  // הייתה כאן תקרה (4 שעות) ולא הייתה רצפה, וזה נראה חסר חשיבות: משכים
  // בלתי סבירים הם **8 מתוך 3,642** בכל הצי, 0.2%.
  //
  // ⚠️ אבל המסך מציג את ה**מינימום**, ולכן שמונת הערכים האלה הם *תמיד*
  // מה שמופיע. הרעש הנדיר ביותר הוא בדיוק זה שמקבל כותרת — ואז הוא
  // סותר את "הכרטיס המהיר" שלידו, שנבנה מממוצעים.
  //
  // ⚠️ **וזו אינה מכונית מהירה, זו חתימה.** נמדד: שמונת הערכים הם
  // *בדיוק* שנייה אחת — לא 0.8 ולא 1.3 — ואחריהם פער ואז 6, 7, 9.
  // הסוכן דוגם את ה-PLC כל 1000ms, ולכן MODE שמהבהב למחזור דגימה יחיד
  // מייצר start ו-end במרחק שנייה בדיוק. ריצוד חיישן או קפיצת רגיסטר,
  // שנקלטים נאמנה על ידי גלאי מונע-קצוות.
  //
  // הסף הוא פעמיים מרווח הדגימה: משך שאינו עולה על שני מחזורים אינו
  // ניתן להבחנה מריצוד, ולכן אינו מעבר רכב. ההתפלגות תומכת — האוכלוסייה
  // האמיתית צפופה סביב 240–480 שניות, ואין דבר בין 1 ל-6.
  const MIN_PLAUSIBLE_SECONDS = 2;

  const openStarts = new Map();
  const durations = [];
  const durationsBy = { entry: [], exit: [] };
  // ⚠️ **נספר, לא נזרק בשקט.** סינון שאיש אינו רואה הוא בדיוק איך מספר
  // שגוי הופך למספר שנעלם — ואז אי אפשר לדעת אם האתר נקי או שהמסנן
  ...
  for (const op of ops) {
    ...
    if (op.start_end === "start") {
      ...
      // אותה בשקט, ואז ה-end הבא נמדד מול ההתחלה **המאוחרת** — כלומר משך
    ...
    if (start !== undefined) {
      ...
      if (seconds >= MIN_PLAUSIBLE_SECONDS && seconds < 4 * 3600) {
  ...
  return {
    ...
    // ============================================================
    // ⚠️ מה **לא** נכנס למדדים, ולמה זה יוצא למסך
    // ============================================================
    // סינון שקט הוא איך מספר שגוי הופך למספר שנעלם: אי אפשר להבדיל בין
    // "האתר נקי" לבין "המסנן עובד קשה". שני המונים האלה הם המדד לאיכות
    // הנתונים עצמם, ולא לביצועי האתר.
    //
