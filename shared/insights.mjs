...
export function computeInsights({ ops, errorRows, maintRows, windows, from, to, siteNames }) {
  ...
  // ===== שיאים =====
  // ==========================================================
  // שני הימים העמוסים, לא אחד
  // ==========================================================
  // יום שיא בודד אינו אומר אם הוא חריג או שגרה. 17 פעולות ביום אחד הוא סיפור
  // אחר לגמרי כשהשני הוא 16 (זה הקצב) מאשר כשהשני הוא 7 (זה אירוע).
  //
  // ⚠️ שובר השוויון הוא **התאריך המוקדם**, ולא סדר ה-Map. שני ימים עם אותו
  // מספר היו מתחלפים בין ריצות לפי סדר ההגעה של השורות — מספר שמשתנה בלי
  // שהנתונים השתנו הוא בדיוק מה שגורם לאבד אמון במסך.
  const dayLabel = (date) => {
    const d = new Date(`${date}T12:00:00`);
    return `${d.getDate()}.${d.getMonth() + 1} (${WEEKDAY_LABELS[d.getDay()]})`;
  };
  const busiestDays = [...byDay]
    .map(([date, operations]) => ({ date, operations, label: dayLabel(date) }))
    .sort((a, b) => b.operations - a.operations || (a.date < b.date ? -1 : 1))
    .slice(0, 2);
