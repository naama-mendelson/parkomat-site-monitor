...
try {
    ...
    if ($loop) {
        # ============================================================
        # ⚠️ קוד ישן הוא הסיבה הנפוצה ביותר לקריסה בלולאה
        # ============================================================
        # נמדד ב-06/09/2026: 19 משתמשי `agent` נוצרו במסד, בעוד שהקונטיינר
        # הריץ קוד ישן שאוסר את הדרגה הזו. בכל עלייה הוא ניסה להחיל
        # `CHECK (role IN ('operator','manager'))`, נכשל, וקרס — 346 פעמים,
        # 6.6 שעות בלי קליטה.
        #
        # ⚠️ **והכפתור הראה את השגיאה ולא את הסיבה.** "צלמי ושלחי" הוא
        # מבוי סתום למי שעומד מול המכונה בשתיים בלילה. השוואה מול origin
        # הופכת את זה ל"הקוד שלך מאחור, הריצי את זה".
        try {
            git -C $Root fetch --quiet 2>$null
            $local  = (git -C $Root rev-parse HEAD 2>$null)
            $remote = (git -C $Root rev-parse "@{u}" 2>$null)
            # ⚠️ **סופרים קומיטים, לא משווים hash.** `$local -ne $remote` נכון
            # גם כשהמכונה **לפני** origin — וזה המצב הרגיל על מחשב פיתוח.
            # הגרסה הראשונה כאן הדפיסה "מאחור ב-0 קומיטים", כלומר הודעה
            # שגויה בדיוק ברגע שסומכים עליה. נתפס בבדיקה, לא בקריאה.
            $behind = 0
            if ($local -and $remote) {
                [void][int]::TryParse((git -C $Root rev-list --count "HEAD..@{u}" 2>$null), [ref]$behind)
            }
            if ($behind -gt 0) {
                Say ("⛔ הקוד כאן מאחור ב-{0} קומיטים — וזו הסיבה הסבירה ביותר." -f $behind) "Red"
                Say "   התיקון:  git pull  ואז  docker compose up -d --build" "Yellow"
            } elseif ($local -and $remote) {
                Say "הקוד כאן מעודכן מול origin — הסיבה אינה גרסה ישנה." "Gray"
            }
        } catch {
            # ⚠️ בלי רשת או בלי git — לא מפילים את הסקריפט. הלוגים למטה
            # עדיין שווים יותר מכלום, וזו כל מטרת הכפתור.
            Say "לא ניתן לבדוק את גרסת הקוד (אין רשת או git)." "DarkGray"
        }
