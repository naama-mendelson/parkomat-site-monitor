...
//
// ============================================================
// ⚠️ קוד אתר הוא **מילה**, לא רצף תווים כלשהו
// ============================================================
// `q.includes(code)` היה התאמת תת-מחרוזת, וכל קודי האתרים הם ארבע ספרות
// (1284, 1343, 1348, ...). לכן:
//
//   • "כרטיס 1284 מפיל את האתר" נקשר לאתר **1284** — מספר כרטיס שנקרא
//     כקוד אתר, והבוט ענה בביטחון על אתר אחר לגמרי.
//   • כל מספר ארוך יותר שמכיל קוד — טלפון, מספר הודעה, חותם — התאים.
//     "0501284..." מכיל 1284.
//
// ⚠️ וזו התשובה הגרועה ביותר האפשרית: לא שגיאה ולא "לא הבנתי", אלא
// **נתונים נכונים על האתר הלא נכון**. אין שום סימן במסך שזה קרה.
function siteFromText(t, sites) {
  // מספר שבא מיד אחרי "כרטיס"/"card" הוא מספר כרטיס. מוסתר לפני החיפוש
  // כדי שלא ייקרא כקוד אתר.
  const q = norm(t).replace(/(כרטיס|card)( מספר| מס| number| no)? ?#? ?[0-9]+/gi, "$1 ");

  // גבול מילה סביב הקוד: ספרה משני צדדיו פוסלת.  אינו מספיק — הוא
  // אמנם חוסם "01284" אבל לא נותן שליטה על מפרידים בעברית.
  const hasCode = (code) => {
    const c = norm(code);
    if (!c) return false;
    let i = q.indexOf(c);
    while (i !== -1) {
      const before = q[i - 1], after = q[i + c.length];
  ...
  const hits = sites.filter((s) => {
    ...
    return hasCode(s.code) || (n && q.includes(n));
  });
