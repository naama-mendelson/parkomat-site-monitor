...
(async () => {
  ...
  // ============================================================
  // ⚠️ provision-agent — ושתי הבדיקות עונות על שתי שאלות שונות
  // ============================================================
  // הראשונה היא **האם היא בכלל פרוסה**. פריסת Edge Function אינה נוסעת
  // ב-git: שינוי שנדחף ל-main אינו בייצור עד שמישהו מריץ פקודה, וזה
  // בדיוק איך שבאג ה-GRANT ב-invite-user שרד — הוא נמצא רק מפני שהשער
  // קורא לפונקציה ה**פרוסה** ולא לקוד שבמאגר.
  //
  // ⚠️ וכאן זה חמור יותר, כי הדשבורד כבר קורא לה בכל הרשמת אתר: בלי
  // פריסה, כל אתר חדש נרשם **בלי זהות** ולא יוכל לדווח לעולם. 404 כאן
  // הוא "שכחו לפרוס", ולא "הרשאות".
  //
  // ⚠️ בקר מקבל 403 **ולא** 404: הבדיקה מתבצעת לפני שהפונקציה נוגעת
  // בקוד האתר, ולכן קוד שאינו קיים עדיין מחזיר 403. זה מכוון — התפקיד
  // נבדק ראשון, אחרת אפשר היה למנות אתרים לפי הודעות השגיאה.
  const provStatus = await edge(oprFresh, "provision-agent", { code: "__none__" });
  add("⚠️ provision-agent פרוסה (404 = לא נפרסה)", provStatus !== 404, true);
  add("⚠️ בקר אינו מנפיק זהות סוכן", provStatus, 403);
