...
async function main() {
  ...
  try {
    ...
    // ------------------------------------------------------------
    // 3. תחזוקה **מהבקר** מוחזרת כתחזוקה — הפוך ממה שנבדק כאן קודם
    // ------------------------------------------------------------
    // ⚠️ השער דרש "תחזוקה אינה מוקמת מחדש", בנימוק שחלון תחזוקה עשוי היה
    // לפוג. **הנימוק הניח שחלון ידני כותב ל-status_history, והוא אינו כותב**
    // — כל שורה שם מגיעה מהבקר, כלומר `maintenance` הוא MODE 0. הוצאתו
    // החזירה אתר בתחזוקה לתקלה שקדמה לה: מקטע תקלה, אחוז כשל והתראה על
    // אתר שאיש לא נגע בו. תוקן ב-ingest.postgres.sql ונבדק על Postgres
    // מקומי (17/09/2026); השער מיושר לכלל הנכון.
    check("הוקם: אתר שהיה בתחזוקה ואז נותק",
      (await putIntoNoCommAfter("maintenance")) === "no_comm");
    await beat();
    check("⚠️ תחזוקה מהבקר מוחזרת כתחזוקה, ולא כמצב שקדם לה", (await statusOf()) === "maintenance",
