...
const GATES = [
  ...
  // ⚠️ מנגנון אחר לגמרי מזה שמעליו: שם השרת מדווח על עצמו, כאן **האתר**
  // מדווח על עצמו ו-pg_cron מבחין בשתיקתו. זה מה שיחליף את הצוואה של
  // MQTT ביום שה-master יורד.
  { name: "check-agent-heartbeat", what: "דופק הסוכן — ומי שמבחין בשתיקתו" },
  { name: "check-silent-sites",  what: "אתר בודד שמנותק שעות — מה ש-detect_blackout מפספס" },
  // ⚠️ ההחלטה "רק תקלה באתר" — ההתראות המערכתיות כבויות מאחורי דגל, ולא
  // מפני שמשהו לא נפרס. השער קורא את הפונקציה **שבייצור**, לא את הקובץ.
  { name: "check-alert-gate",    what: "התראות מערכתיות כבויות — והדגל באמת חוסם" },
  { name: "check-controller-replace", what: "הוחלף בקר — והמונה הבלתי-הפיך לא זז" },
  ...
  // ⚠️ ואותה טענה בדיוק על המסלול הישיר, שבו היא **לא** התקיימה: אפס
  ...
  // ⚠️ events הוא חוזה האירועים (כלל 8), ו-SSE ו-Realtime שניהם קוראים
  // ממנו. המסלול הישיר לא כתב שם כלום — כלומר מסך שאינו זז.
  { name: "check-direct-events", what: "המסלול הישיר כותב אירועים, ובאותו מבנה" },
