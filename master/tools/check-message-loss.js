...
(async () => {
  ...
  // ============================================================
  // ⚠️ אחוזון נמוך ולא חציון — והחציון הפליל אתרים תמימים
  // ============================================================
  // ההפרש שנמדד כאן הוא **היסט שעון + השהיית מסירה**, ושתיהן חיוביות.
  // חציון על אתר שקט עם 16 אירועים ב-48 שעות נגרר בקלות על ידי שש
  // הודעות שהמתינו בתור.
  //
  // ⚠️ **וזה קרה, ונמדד:** ב-07/09/2026 השער דיווח על 1416 סטייה של
  // 7,072 שניות, ועל 1275 ו-1415 סטיות דומות. הבדיקה בפועל: כל ההודעות
  // האיטיות בשלושת האתרים נקלטו **באותו חלון של ארבע דקות** — 06/09
  // בין 06:44 ל-06:48 — וכל שאר ההודעות שלהם היו 3 שניות. זה השרת
  // שחזר לאוויר אחרי 6.6 שעות השבתה וריקן את התור של HiveMQ.
  //
  // שעון סוחף אינו נראה כך: הוא מזיז **כל** הודעה, בעקביות. תור נראה
  // כמו קבוצה צפופה בחלון קליטה אחד ואפס בשאר הזמן.
  //
  // ⚠️ ולכן האומדן הוא ה**מינימום המעשי** (אחוזון 10): השהיית רשת ותור
  // היא תמיד תוספת חיובית, ולכן ההודעה שהתעכבה הכי פחות היא הקירוב
  // הטוב ביותר להיסט האמיתי. זו אותה שיטה שבה NTP מסנן מדידות.
  const pLow = (arr) => {
    const q = [...arr].sort((x, y) => x - y);
    return q[Math.min(q.length - 1, Math.floor(q.length * 0.1))];
  };
  const perSite = [...bySite.entries()]
    .map(([code, v]) => ({ code, m: pLow(v), med: median(v), n: v.length,
                           late: v.filter((x) => x > 300).length }));
  if (!perSite.length) { console.log("   (אין אירועים בטווח)"); }
    ...
    console.log(`   בסיס המערכת: ${Math.round(base)} שניות  (אחוזון 10 — עמיד לתור)`);

    // ⚠️ מסירה איטית נאמרת **בנפרד ובשמה**, ולא כ"שעון". היא אמיתית
    // וראויה לדיווח — אבל היא תכונה של הרשת ושל השרת, לא של האתר, וטיפול
    // בה הוא במקום אחר לגמרי. איחוד השניים שלח לחפש שעון בחניון תקין.
    const backlog = perSite.filter((x) => x.late > 0);
    if (backlog.length) {
      const total = backlog.reduce((n, x) => n + x.late, 0);
      console.log(`   ⚑ מסירה מאוחרת: ${total} הודעות מעל 300ש' ב-${backlog.length} אתרים` +
                  ` — ${backlog.length >= 3 ? "חתימה של תור אחרי נפילת שרת, לא של שעונים" : "בדוק את האתרים האלה"}`);
    }
    let odd = 0;
