...
async function run(client) {
  failures = 0;
  {
    ...
    // ⚠️ **שלוש, ולא ארבע — והשער עצמו היה אדום על כך.** ההתראה "השרת חדל
    // לדווח" הוסרה ב-22/09/2026: `master` יצא משימוש, אות החיים שלו לא
    // יתעדכן עוד לעולם, והיא הייתה מדווחת "ייתכן שהקליטה מושבתת" כל עשר
    // דקות — בזמן שהקליטה עובדת מצוין ישירות מול Supabase. הקוד מתעד את
    // ההסרה בסעיף 1 של `check_ingestion_health`; השער נשאר מאחור.
    check("שלוש ההתראות קיימות בייצור", calls === 3, `נמצאו ${calls}`);
    ...
    // ============================================================
    // ההתראות המערכתיות הן על **המערכת שלנו** (הודעות נזרקות, חשכה
    // כללית — ועד 22/09 גם "השרת חדל לדווח", שהוסרה עם `master`), נמדדו
    // כ-4–6 בשבוע, והוחלט לא לקבל אותן. האחרונה שואלת שאלה אחרת לגמרי:
    // **אתר של לקוח מנותק שעות.**
    //
    ...
    // גם את זה.
    check("⚠️ שתי המערכתיות מגודרות בדגל המערכת", bySys === 2, `${bySys}`);
