...
const TRANSIENT_CODES = new Set([
  ...
  "55P03",
  // ============================================================
  // ⚠️ 40P01 / 40001 — התנגשות טרנזקציות, וזה **הכשל שאיבד תקלה אמיתית**
  // ============================================================
  // 40P01 = deadlock_detected, 40001 = serialization_failure. שניהם אומרים
  // דבר אחד: שתי טרנזקציות התנגשו, Postgres הכריע והפיל אחת מהן. הטרנזקציה
  // מתגלגלת **במלואה**, ולכן ניסיון חוזר הוא התגובה הנכונה — וזו גם ההמלצה
  // המפורשת בתיעוד של Postgres.
  //
  // ⚠️ **בלעדיהם ההודעה אבדה, ולא רק התעכבה.** deadlock שאינו מסווג כחולף
  // מדלג על חמשת הניסיונות של השכבה הזו, נופל ישר ל-5 הניסיונות המהירים של
  // ה-dispatcher (3.75 שניות בסך הכול), וזה מסתיים ב-`return` — כלומר PUBACK,
  // וההודעה נמחקת מ-HiveMQ לתמיד.
  //
  // ⚠️ **וזה נמדד, לא הוסק.** ב-23.08 אבדה הודעת תקלה מאתר 1284: HiveMQ
  // אישר אותה ב-RC:0 והשרת לא כתב אותה. הפער בין השידור לרישום היה **7.13
  // שניות** — בדיוק מסלול הכשל-המהיר. באותו יום נמדדו deadlock-ים בין
  // `ALTER TABLE` שרץ ב-db.init() (ACCESS EXCLUSIVE על status_history) לבין
  // הקליטה שכתבה באותו רגע. ה-ALTER תוקן מאז לרוץ פעם אחת בלבד, אבל
  // **הסיווג הזה הוא ההגנה השנייה**: התנגשות תמיד תיתכן.
  //
  // ⚠️ ובטוח לניסיון חוזר כאן מאותה סיבה שכל השאר בטוחים: הקליטה אידמפוטנטית
  // — dedup על reported_at שאינו משתנה, ושומרי backfill/no-change שמדלגים
  // על מה שכבר נכתב. ה-backoff המעריכי (250ms→4s) גם מפזר את הניסיון
  // החוזר, כך ששתי הטרנזקציות אינן מתנגשות שוב מיד.
  "40P01", "40001",
]);
...
function init() {
  ...
  initPromise = retryTransient(async () => {
    ...
    try {
      ...
      // ============================================================
      // הקליטה ב-SQL — נבנית בפרוסות, ואינה בשימוש עדיין
      // ============================================================
      // ⚠️ הקובץ מגדיר פונקציות בלבד; שום דבר אינו קורא להן היום. הוא
      // מוחל כאן כדי ששער ההשוואה יוכל להריץ אותן מול המסלול הקיים —
      // פונקציה שאינה מוחלת אינה ניתנת להשוואה, וההשוואה היא כל התכלית.
      //
      // ⚠️ **וכישלון כאן כן מפיל את העלייה**, בניגוד ל-cron למטה: זו אינה
      // הרחבה שעשויה לא להיות מותקנת אלא קוד שלנו. שגיאת תחביר שנבלעת
      // בשקט הייתה משאירה את השער משווה מול גרסה ישנה של הפונקציה, וזה
      // בדיוק המצב שבו שער ירוק אינו אומר דבר.
      const ingest = fs.readFileSync(path.join(__dirname, "ingest.postgres.sql"), "utf8");
      await setup.query(ingest);

      // ============================================================
      // ⚠️ הרמזור, שעות השירות ו-FixFlow — מוחלים כאן, ולא רק ביד
      // ============================================================
      // שלושת הקבצים האלה נכתבו אחרי שהסדר כאן נקבע, ואף אחד מהם לא נוסף.
      // שניים הוחלו ביד בלבד ואחד בכלי ייעודי — כלומר מסד חדש (שחזור, מסד
      // בדיקות, יציאה מ-Supabase) עלה בלי `tl_board` ובלי `site_uptime_service`,
      // והדשבורד חזר בשקט ל-24/7. וגם בייצור: תיקון בקובץ לא הגיע ל-deploy.ps1,
      // שהוא הנתיב המתועד להחלת SQL.
      //
      // ⚠️ **הסדר מחייב:** traffic-light לפני service-hours (שקוראת את טבלאות
      // הלוח), ושניהם אחרי security ו-functions (`app.is_active_user`,
