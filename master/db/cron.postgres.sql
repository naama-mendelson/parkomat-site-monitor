...
BEGIN
  ...
  WITH expected AS (
    ...
    SELECT s.id, s.code, s.status, s.last_seen, a.seen_at
      FROM sites s
...
-- ============================================================
-- app.detect_silent_sites — אתר בודד שמנותק זמן רב
-- ============================================================
-- ⚠️ **זה לא גלאי שקט, וההבדל הוא כל העניין.** שקט נפסל
-- כאות במדידה: הסוכן משדר על שינוי בלבד, ולכן פערים של
-- 61–68 שעות בין הודעות הם שגרה בחניון שקט — סף שלא מצפצף
-- עליהם ארוך מהנפילות שאנו מנסים לתפוס.
--
-- לכן האות כאן אינו העדר הודעות אלא **הודעה שהתקבלה**: הצוואה
-- של הגשר פותחת מקטע no_comm תוך 90 שניות, והשאלה היא כמה זמן
-- המקטע הזה נשאר פתוח. חניון שקט אינו מייצר מקטע כזה כלל,
-- ולכן אין כאן התנגשות בין "שקט" ל"מת".
--
-- ⚠️ **וזה נמדד על ההיסטוריה, לא נוחש.** ב-48 יום היו 270 מקטעי
-- no_comm, רובם ריצוד של שניות. בסף 6 שעות עוברים **7 בלבד**,
-- כלומר כהתראה אחת לשבוע — וכל שבעתם אמיתיים:
--
--     2439   88.9ש      2438   71.0ש      1284   24.2ש
--     1376   29.8ש      2438   68.2ש      1348    7.5ש
--     3452  220.2ש   ← תשעה ימים, ואיש לא ידע
--
-- סה"כ 510 שעות של אתרים מושבתים, בשישה אתרים, **בלי התראה אחת**.
--
-- ⚠️ ולמה detect_blackout לא תפס את 3452: הוא בודק MAX(received_at)
-- על **כל** האתרים יחד. בזמן ש-3452 שתק תשעה ימים, 20 אתרים
-- אחרים דיווחו כרגיל — המערכת לא היתה חשוכה, ולכן הגלאי
-- שתק בצדק. שני הגלאיים עונים על שתי שאלות שונות ואינם מחליפים
-- זה את זה.
--
-- ⚠️ מקטעים שסומנו כניסוי (excluded_at) מווצאים, והמצב נקרא דרך
-- COALESCE(reclassified_to, status) — כמו בכל קורא אחר של הטבלה.
-- מקטע שסווג מחדש כתחזוקה איננו ניתוק, והתראה עליו היא התראת שווא.
CREATE OR REPLACE FUNCTION app.detect_silent_sites(
  p_hours numeric DEFAULT 6,
  p_at    timestamptz DEFAULT now()
)
RETURNS TABLE (site_code text, site_name text, quiet_hours numeric, since text)
LANGUAGE sql
STABLE
SECURITY DEFINER
SET search_path = public, app, pg_temp
AS $fn$
  SELECT s.code,
         s.site_name,
         ROUND((EXTRACT(EPOCH FROM (p_at - h.started_at::timestamptz)) / 3600)::numeric, 1),
         h.started_at
    FROM status_history h
    JOIN sites s ON s.id = h.site_id
   WHERE h.excluded_at IS NULL
     -- ⚠️ **חפיפה, ולא `ended_at IS NULL`** — וזה הבדל של יכולת
     -- הוכחה, לא של סגנון. הגרסה הראשונה שאלה "פתוח עכשיו?",
     -- ולכן הרצה על תאריך היסטורי החזירה כלום: הניתוק של 3452
     -- היה פתוח ב-20/08 אבל נסגר מאז. כלומר **אי אפשר היה להוכיח
     -- שהגלאי תופס את המקרה שבגללו נבנה** — גלאי שאינו ניתן
     -- לבדיקה מול היסטוריה הוא גלאי שאיש אינו יודע מה הוא תופס.
     --
     -- ב-p_at = now() התנאי הזה שקול לחלוטין לקודם (אין מקטע
     -- שנסגר בעתיד), אז ההתנהגות בייצור לא השתנתה — רק הפכה
     -- ניתנת לבדיקה. אותה צורת חפיפה ש-insightsDirect משתמש בה.
     AND (h.ended_at IS NULL OR h.ended_at::timestamptz > p_at)
     AND COALESCE(h.reclassified_to, h.status) = 'no_comm'
     AND h.started_at::timestamptz <= p_at - make_interval(secs => p_hours * 3600)
...
DECLARE
  ...
  -- ============================================================
  -- ⚠️ ודגל שני, נפרד — כי הדגל האחד היה גס מדי
  -- ============================================================
  -- שלוש ההתראות הראשונות הן על **המערכת שלנו**: השרת חדל לדווח,
  -- הודעות נזרקות, חשכה כללית. הן נמדדו כ-4–6 בשבוע והוחלט לא לקבל אותן.
  --
  -- ⚠️ **הרביעית שואלת שאלה אחרת לגמרי: אתר של לקוח מנותק שעות.**
  -- דגל אחד לשתי השאלות פירושו שכיבוי הרעש מכבה גם את זה.
  --
  -- ⚠️ **נמדד ב-09/09/2026, והמחיר היה קונקרטי:** שישה אתרים היו
  -- מנותקים בין 9 ל-15.5 שעות בעקבות אתחול של עדכון Windows, ומה שגילה
  -- את זה היה מבט מקרי במסך. חמישה מהם היו חוצים את סף שש השעות ומייצרים
  -- התראה ב-06:00 — שעתיים לפני שמישהו הבחין.
  --
  -- ⚠️ **והכיול כבר מסנן את הרעש הכרוני:** אתר 1326 מנתק 19 פעמים
  -- בחודש, 12 מהן חוזרות תוך עשר דקות. באותו לילה הוא היה מנותק 11
  -- דקות — **ולא היה מצפצף**. הסף של שש שעות נבחר בדיוק בשביל זה.
  --
  -- מדוד על ההיסטוריה: 3.2 התראות בשבוע.
  --
  -- להדליק:
  --   INSERT INTO settings (key, value, updated_at)
  --   VALUES ('silent_site_alerts_enabled','true', now()::text)
  --   ON CONFLICT (key) DO UPDATE SET value = 'true';
  v_silent_on boolean := COALESCE(
                (SELECT value FROM settings WHERE key = 'silent_site_alerts_enabled'),
                'false') = 'true';
BEGIN
  -- ============================================================
  ...
  -- ============================================================
  -- 4. אתר בודד שמנותק שעות
  -- ============================================================
  -- ⚠️ סעיף 3 תופס חשכה מערכתית בלבד. אתר אחד שנפל בזמן
  -- שהשאר מדווחים עובר מתחת לכל הרדארים — וזה קרה:
  -- 220 שעות באתר 3452.
  FOR v_sil IN SELECT * FROM app.detect_silent_sites() LOOP
    -- ⚠️ **מפתח השתקה לכל אתר בנפרד, ולא אחד גלובלי.** מפתח
    -- משותף היה גורם להתראה על אתר אחד להשתיק את כל השאר
    -- לשש שעות — כלומר נפילה רחבה היתה מדווחת כאתר אחד.
    -- זה בדיוק הפער השקט שהסעיף הזה נולד כדי לסגור.
    SELECT value INTO v_last FROM settings
      WHERE key = 'alert_last_silent_' || v_sil.site_code;

    -- ============================================================
    -- דה-דופ של 24 שעות — נמדד, לא נבחר בתחושה
    -- ============================================================
    -- ניתוק נמשך ימים והמקטע נשאר פתוח כל הזמן הזה, אז השאלה
    -- אינה "האם להתריע" אלא "כל כמה להזכיר". נמדד על 48 ימי
    -- היסטוריה, עם התראה ראשונה ב-6 שעות:
    --
    --      6ש  →  81 התראות  =  11.7 לשבוע
    --     12ש  →  42            =   6.1
    --     24ש  →  22            =   3.2   ← נבחר
    --     48ש  →  14            =   2.0
    --
    -- 24 שעות נותן לניתוק של 3452 תשע תזכורות — אחת ליום שבו האתר
    -- היה מושבת, מה שנשמע נכון — ומוריד את הרעש מ-11.7 ל-3.2 לשבוע.
    --
    ...
    IF v_last IS NULL OR EXTRACT(EPOCH FROM (now() - v_last::timestamptz)) / 3600 > 24 THEN
      -- ⚠️ **הדגל של האתר השקט, ולא זה של המערכת** — ראה v_silent_on.
      -- זו השאלה "אתר של לקוח מנותק", והיא נפרדת מ"המערכת שלנו חולה".
      v_req := CASE WHEN v_silent_on THEN app.send_push(
      ...
      -- התראה שמעולם לא יצאה.
      IF v_req IS NOT NULL THEN
        INSERT INTO settings (key, value, updated_at)
        VALUES ('alert_last_silent_' || v_sil.site_code, v_now, v_now)
          ON CONFLICT (key) DO UPDATE SET value = v_now, updated_at = v_now;
      END IF;
      RETURN QUERY SELECT 'silent_site'::text,
...
-- ============================================================
-- ⚠️ מי שמחיל רק את הפונקציה — לעצור כאן
-- ============================================================
-- הפונקציה נגמרת ב-`$fn$;` שמעל. מיד אחריו מגיע בלוק `DO`
-- ש**מבטל את התזמון**, וה-`cron.schedule` שמחזיר אותו יושב עוד
-- כמה שורות מתחת. פרוסה שנעצרת באמצע מוחקת את המשימה
-- מהלוח ואינה מחזירה אותה — וזה קרה: 22 דקות בלי בדיקת
-- בריאות בכלל, בלי שום שגיאה בשום מקום.
--
-- ⚠️ והמשמר של הפרוסה עבר בסיבה אחרת מזו שהתכוונה לה: הוא
-- דרש סיום ב-`$$;`, וזה בדיוק מה שמסיים את בלוק ה-`DO`.
-- הסיום של הפונקציה הוא `$fn$;`.
--
-- `check-alert-gate` הוא מה שתופס את זה היום.
DO $$
BEGIN
