...
CREATE TABLE IF NOT EXISTS sites (
  ...
  -- ============================================================
  -- ⚠️ ספריית התקלות של האתר ב-FixFlow — `"מערכת|פרופיל"`
  -- ============================================================
  -- שדה **הפיילוט** היחיד במסד. הכלל המקורי היה "אין טבלה ואין עמודה", כדי
  -- שהסרת FixFlow תהיה מחיקת תיקייה. הכלל נשבר כאן במודע, ומסיבה שנמדדה:
  --
  -- ⚠️ **שיוך אינו ניתן לגזירה, לא בעיקרון ולא בפועל.** ללולק לבדה שישה
  -- פרופילי מצבט; FixFlow מתייקת 5 אתרים ל-`שאטל מצבט x` שיש בו 0 מסמכים
  -- בזמן ש-25 המסמכים יושבים ב-`שאטל מצבט שמסובבת במעלית` שיש בו אתר אחד.
  -- שלוש שיטות גזירה נוסו ונמדדו — שם, סוג מכונה, וניתוח נוסח התקלות —
  -- וכולן נכשלו או סירבו.
  --
  -- ⚠️ **ולמה במסד ולא בקובץ בריפו.** המפה נבנית בכלי על מחשב הפיתוח. כלומר
  -- המשתמשת אינה יכולה לשנות קישור, ואתר שנרשם היום נשאר בלי קישור עד
  -- שמישהו יריץ כלי וידחוף קוד. זה הפך את הקישור לדבר שחי מחוץ למחזור החיים
  -- של האתר — וזה הפגם, לא שני האתרים שלא נפתרו.
  --
...
-- ============================================================
-- ⚠️ alive — הדופק של הסוכן, ולא של הנתונים
-- ============================================================
-- `last_seen` אומר "מתי הגיעה הודעה אחרונה", והוא **אינו** מספיק
-- לזיהוי ניתוק: הסוכן משדר על שינוי MODE בלבד, ונמדד שאתר שקט
-- 61–68 שעות ברוטינה. סף שיימנע רעש חייב להיות ארוך מהתקלה שמחפשים.
--
-- השורה כאן נכתבת בכל קריאה ל-`ingest_batch`, **כולל אצווה ריקה** —
-- כלומר היא אומרת "הסוכן דיבר איתנו", בלי קשר לשאלה אם היה לו מה לומר.
--
-- ⚠️ **טבלה אחת לכולם, שורה לאתר — ולא טבלה לכל אתר.**
-- טבלה לאתר פירושה `CREATE TABLE` בכל הרשמת אתר, כלומר DDL בתוך פעולה
-- שמנהל מריץ מהדפדפן — והשאלה היחידה ששואלים אותה, *"מי שתק?"*, הייתה
-- הופכת מסריקה אחת ל-18 שאילתות שגדלות עם הצי.
--
-- ⚠️ **ולמה טבלה ולא עמודה על `sites` — זו הנעילה.**
-- `applyStateChange` נועל את שורת האתר ב-`SELECT … FOR UPDATE` בכל קליטה,
-- והנעילה מוחזקת עד סוף הטרנזקציה. פעימה שכותבת לאותה שורה מתחרה על
-- אותה נעילה עם הקליטה עצמה — וככל שהדופק מהיר יותר, כן ההתנגשות תכופה
-- יותר. טבלה נפרדת מנתקת את שתי הכתיבות לגמרי, וזה מה שמאפשר לקצר את
-- מרווח הפעימה בלי לגעת בקליטה.
--
-- ⚠️ **והיא אינה גדלה לעולם.** `ON CONFLICT DO UPDATE` דורס את אותה שורה,
-- אז 18 שורות נשארות 18 לנצח. אין מה לגזום ואין משימת `pg_cron` לניקוי —
-- בניגוד ל-`events`, שבלי גיזום גדלה ללא גבול.
--
-- ⚠️ **`timestamptz` ולא TEXT — חריגה מודעת משאר הקובץ.**
-- הכלל "תאריכים הם TEXT" קיים כדי שסינון לקסיקלי ינצל אינדקס על טבלאות
-- גדולות. כאן יש שורה לאתר, הסריקה קוראת את כולן בכל מקרה, ואין מה
-- לסרוק בטווח. TEXT היה מחייב `::timestamptz` בכל השוואה — עלות בלי
-- תמורה, ומקור לשגיאת אזור זמן בשקט.
--
-- `agent_version` עונה על פער מתועד: *"אין דיווח גרסה בשום topic, אי
-- אפשר לדעת מרחוק מי קיבל מה"*. אחרי התקנה ב-16 אתרים זו שאילתה אחת.
...
-- ⚠️ `ON DELETE CASCADE` ולא `SET NULL` — בניגוד ל-`events`. שם מחיקת אתר
-- **מפרסמת** אירוע, ו-CASCADE היה מוחק את ההודעה על המחיקה יחד עם האתר.
-- כאן הפוך: "מתי נראה לאחרונה סוכן של אתר שאינו קיים" אינו מידע — הוא
-- שורה שהסריקה תצטרך לדלג עליה לנצח.

-- ============================================================
-- ⚠️ תצלום המערכות — לאתר עם שתי מערכות בבקר אחד
-- ============================================================
-- אתר פלורנטין מריץ שני ParkManager על אותו בקר. מצב האתר נשאר **אחד**
-- (הטוב מבין השתיים, מאוחד כבר בסוכן), ולכן `ingest_state`, המקטעים,
-- הזמינות ואחוז הכשל אינם משתנים בגללו **בכלל**. מה שנוסף הוא פירוט
-- לתצוגה: איזו מערכת במצב מה, ואיזה רכב בכל אחת.
--
-- ⚠️ **וזה כאן ולא ב-`sites`, מאותה סיבה שהטבלה הזו נולדה.** `sites`
-- מוחזקת ב-`SELECT … FOR UPDATE` לאורך כל טרנזקציית הקליטה; כתיבה של
-- מצב חי לתוכה כל 60 שניות הייתה מתחרה בקליטה עצמה. `alive` מנותקת
-- ממנה, נדרסת בכל פעימה, ולעולם אינה גדלה.
--
-- ⚠️ **NULL פירושו "מערכת אחת", ולא "לא ידוע".** זו ההבחנה שמאפשרת
-- לכרטיס לדעת אם להציג שבב אחד או שניים, בלי שדה נוסף שאפשר לסתור.
ALTER TABLE alive ADD COLUMN IF NOT EXISTS systems jsonb;

-- ⚠️ העמודה שקדמה לטבלה — נמחקת, ובכוונה.
-- שני מקומות שעונים "מתי הסוכן דיבר איתנו לאחרונה" הם שני מקומות שיכולים
-- להסכים. נמדד לפני המחיקה: 18 אתרים, אפס ערכים — המסלול הישיר כבוי בכל
-- האתרים, ולכן לא אבד דבר.
ALTER TABLE sites DROP COLUMN IF EXISTS agent_seen_at;
