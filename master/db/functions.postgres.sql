...
RETURNS TABLE (
  ...
  systems                     jsonb,
  -- ============================================================
  -- ⚠️ מתי הפירוט נקרא — בלעדיו הוא נתון בלי תאריך תפוגה
  -- ============================================================
  -- `systems` מגיע מ-`alive`, כלומר מהפעימה האחרונה. כשהפעימה נעצרת השורה
  -- **נשארת כפי שהייתה**, ואין בה דבר שמעיד על כך.
  --
  -- ⚠️ נמדד בפלורנטין ב-15/09: `sites.status` היה `operating` (עדכני, הגיע
  -- ב-MQTT) בזמן ש-`systems` אמר `[מוכן, תחזוקה]` — מהפעימה של **אתמול
  -- ב-22:58**, יותר מעשר שעות קודם. הכרטיס הציג את שניהם זה לצד זה כאילו
  -- הם מתארים את אותו רגע. אין שום קשר ביניהם, וזה נראה כמו באג בתצוגה.
  --
  -- ⚠️ ולמה חותמת ולא סינון כאן: אתר דו-מערכתי שהפעימה שלו נעצרה עדיין
  -- **הוא** דו-מערכתי. החזרת NULL הייתה מוחקת גם את העובדה הזו, והמסך לא
  -- היה יכול לומר "הפירוט אינו זמין" — הוא פשוט לא היה יודע שיש מה להציג.
  systems_seen_at             text
)
LANGUAGE sql
STABLE
AS $$
...
COMMENT ON FUNCTION public.site_globals(integer[]) IS
  ...
  'פעולות מאז התקלה, ותחזוקה פעילה, ופירוט המערכות עם חותמת הזמן שבה נקרא. '
  'תרגום של getAllSitesGlobals. ללא טווח תאריכים.';
