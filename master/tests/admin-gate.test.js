...
test("⚠️ במצב ישיר ההרשאה היא תפקיד, ולא הקוד המשותף", () => {
  ...
  // ============================================================
  // הגרסה הקודמת החזיקה את הנעילה ב-useState (`dismissed`), ולכן כפתור
  // "נעל" **לא עשה כלום**: `onClose()` פירק את AdminPanel מיד אחריו, וה-
  // state מת יחד איתו. אחסון שנשמר מחוץ לרכיב הוא כל התיקון.
  assert.match(direct, /!directLocked/, "unlocked אינו מתחשב בנעילה שנשמרה");
  assert.match(src, /isDirectLocked|setDirectLocked/, "הנעילה הישירה אינה נשמרת מחוץ לרכיב");
  assert.ok(
    !/\bdismissed\b/.test(src),
    "`dismissed` חזר — נעילה ב-state של רכיב שנהרס בסגירה אינה נועלת",
  );
});
