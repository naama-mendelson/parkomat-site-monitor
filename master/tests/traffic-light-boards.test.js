...
// ============================================================
// ⚠️ כל שאילתה בכלים חייבת לציין לוח
// ============================================================
// ארבעה כלים נכתבו לפני שהיה לוח שני, ונשארו בלי סינון: היבואן הרובוטי,
// ה-diff, ושני כלי FixFlow. נמדד בהרצה יבשה מול הייצור ב-23/09/2026:
// היבואן הישן היה משנה **281 תאים ב-149 שורות** — "אתר" ו"אחריות" נכתבים
// תחת המפתחות של המכפילים, כי שתי העמודות קיימות בשני הלוחות באותו
// position ו-`byLabel` בחר את השגויה. אחרי הסינון: 4 תאים, כולם אמיתיים.
//
// הבדיקה סטטית בכוונה: את הכלים מריצים ידנית מול הייצור, ואין שום
// הרצה אוטומטית שהייתה תופסת כלי חמישי שנכתב באותה צורה.
test("⚠️ כל שאילתה בכלים שנוגעת בטבלאות הרמזור מציינת לוח", () => {
  const dir = path.join(__dirname, "..", "tools");
  const bad = [];
  let seen = 0;
  for (const f of fs.readdirSync(dir).filter((n) => n.endsWith(".js"))) {
    const src = fs.readFileSync(path.join(dir, f), "utf8");
    // כל template literal בקובץ; ההערות אינן רלוונטיות כי אינן בגרשיים הפוכים.
    for (const m of src.matchAll(/`[^`]*`/g)) {
      const q = m[0];
      if (!/traffic_light_(rows|columns)/.test(q)) continue;
      seen++;
      if (!/\bboard\b/.test(q)) {
        const line = src.slice(0, m.index).split("\n").length;
        bad.push(`${f}:${line}  ${q.replace(/\s+/g, " ").slice(0, 90)}`);
      }
    }
  }
  // ⚠️ רצפה: רגקס ששבר בשקט היה מוצא אפס שאילתות ועובר.
  assert.ok(seen >= 15, `נמצאו רק ${seen} שאילתות — הסריקה עצמה שבורה`);
  assert.deepEqual(bad, [], "שאילתות בלי סינון לוח:\n" + bad.join("\n"));
});
