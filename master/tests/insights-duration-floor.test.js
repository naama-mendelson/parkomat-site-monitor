...
// ============================================================
// ⚠️ מפתח הזיווג — הבאג הגדול יותר, שנמצא רק אחרי שנמדד
// ============================================================
// המפתח היה `אתר|כיוון|כרטיס`, וזה נראה סביר: הוא מונע שיוך בין שני
// רכבים. אבל הוא נשען על הנחה שגויה — שהכרטיס ידוע כבר ב-`start`.
//
// ⚠️ נמדד על הצי, 30 יום: **73 זוגות (2.0%) שבהם הכרטיס בהתחלה ריק
// ובסיום מלא — ובכל 73 המקרים לכיוון הזה בלבד, אף פעם להפך.** זה סדר
// פעולות ולא רעש: המחסום נכנס למצב כניסה, ורק אז הנהג מציג כרטיס.
//
// ⚠️ **וההפסד לא התפלג אחיד: 58 מ-73 באתר 3501.** כלומר אתר שלם שכל
// המדדים שלו חושבו מתת-קבוצה מוטה — רק הפעולות שבהן הכרטיס במקרה כבר
// היה ידוע. אחרי התיקון: 270 → 328 פעולות נמדדות שם.

test("⚠️ כרטיס ריק בהתחלה ומלא בסיום — הפעולה נמדדת", async () => {
  const r = await run([
    op("2026-09-01T10:00:00.000Z", "start", "entry", ""),   // המחסום נפתח
    op("2026-09-01T10:05:00.000Z", "end", "entry", "11"),   // הכרטיס נקרא אחר כך
  ]);

  assert.strictEqual(r.durations.samples, 1,
    "הפעולה אבדה — זה 2% מהפעולות בצי, ו-18% באתר 3501");
  assert.strictEqual(r.durations.averageSeconds, 300);
  assert.strictEqual(r.excluded.cardMismatch, 0);
});

test("⚠️ שני כרטיסים שונים ולא-ריקים — לא נמדד, ונספר", async () => {
  // ההגנה שהמפתח נתן, עכשיו במפורש: התחלה של רכב אחר שהסיום שלה אבד.
  // מדידה כזו הייתה מודדת את המרווח **בין שני רכבים**, לא משך פעולה.
  const r = await run([
    op("2026-09-01T10:00:00.000Z", "start", "entry", "7"),
    op("2026-09-01T10:05:00.000Z", "end", "entry", "11"),
  ]);

  assert.strictEqual(r.durations, null);
  assert.strictEqual(r.excluded.cardMismatch, 1);
});

test("⚠️ אותו כרטיס — נמדד כרגיל, וזה 98% מהמקרים", async () => {
  const r = await run([
    op("2026-09-01T10:00:00.000Z", "start", "exit", "11"),
    op("2026-09-01T10:04:00.000Z", "end", "exit", "11"),
  ]);
  assert.strictEqual(r.durations.samples, 1);
  assert.strictEqual(r.durations.averageSeconds, 240);
});

test("⚠️ כיוונים נפרדים אינם מזדווגים זה עם זה", async () => {
  // המפתח איבד את הכרטיס אך **לא** את הכיוון: כניסה ויציאה הן שתי
  // פעולות מכניות שונות, ומדידה חוצה-כיוונים הייתה חסרת משמעות.
  const r = await run([
    op("2026-09-01T10:00:00.000Z", "start", "entry", "5"),
    op("2026-09-01T10:01:00.000Z", "end", "exit", "5"),
  ]);
  assert.strictEqual(r.durations, null);
});
