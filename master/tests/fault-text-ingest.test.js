...
// ============================================================
// ⚠️ מפרידים אינם "תווים לא חוקיים"
// ============================================================
// הגרסה הקודמת נכתבה דרך כלי עריכה שבלע לוכסן, ו-\s ברגקס הפך ל-s
// ספרותי. התוצאה: ירידות שורה וטאבים נמחקו, ו-"שורה א\nשורה ב" הפך
// ל-"שורה אשורה ב" — מילים דבוקות בלי מפריד, בטקסט שמוצג למפעיל.
//
// ⚠️ ולכן הסינון כאן נבנה מקודי תווים ולא מרגקס: אין בו לוכסן שאפשר
// לאבד.
test("⚠️ ירידת שורה וטאב נשמרים — הם מפרידים, לא זבל", () => {
  assert.equal(
    extractFaultText("error", { faultText: "שורה א\nשורה ב" }),
    "שורה א\nשורה ב");
  assert.equal(extractFaultText("error", { faultText: "א\tב" }), "א\tב");
});

// ============================================================
// ⚠️ טקסט שכולו סימני שאלה
// ============================================================
// הסוכן המתוקן ממיר רגיסטר לא-מזוהה ל-'?'. '?' הוא ASCII נדפס, ולכן
// עובר את הסינון — ואז המפעיל רואה "???" ואנחנו לא רושמים כלום. זו
// בדיוק התוצאה שהמודול קיים כדי למנוע.
test("⚠️ '???' הוא זבל, גם אם ASCII מתיר אותו", () => {
  assert.equal(extractFaultText("error", { faultText: "???" }), null);
  assert.equal(extractFaultText("error", { faultText: "?? ?" }), null);
});

test("⚠️ אבל סימן שאלה בתוך תיאור אמיתי נשאר", () => {
  // "מעלית תקועה?" הוא תיאור לגיטימי. חיתוך גורף היה מוחק אותו.
  assert.equal(
    extractFaultText("error", { faultText: "מעלית תקועה?" }),
    "מעלית תקועה?");
});
