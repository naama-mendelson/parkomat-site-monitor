...
// ============================================================
// ⚠️ **ההחלטה התהפכה פעמיים, וזה הסדר**
// ============================================================
//   1. **פס בראש המסך בלבד** — לא חוסם, וקל מדי לפספס. נאמר מהשטח.
//   2. **שלט מרכזי שנסוג אחרי 25 שניות** — פשרה, בנימוק שמסך קיר שמתאתחל
//      בלילה לא ייתקע על חלון בלי להציג אתרים.
//   3. **שלט שאינו נסוג** (22/09/2026, בקשת בעלת המוצר: "אני רוצה שיהיו
//      חייבים לאשר הפעלת התראות"). מה שהפשרה עלתה התברר בצילום מהשטח:
//      כפתור כתום בכותרת, הקול כבוי, ומסך שעובד יום שלם בלי שתקלה תצלצל
//      בו. **קול שאיש אינו מפעיל אינו "פחות טוב" — הוא לא קיים.**
//
// ⚠️ **המחיר של 3 נשאר אמיתי** — מסך קיר שמתאתחל בלילה יחכה ללחיצה —
// ולכן הבדיקות כאן מקבעות גם את שני הדברים שמקטינים אותו: רקע שקוף
// (רואים שהמערכת עובדת מאחור) והוראה להרשאת Sound, שמייתרת את החלון לגמרי.
const barCode = BAR.replace(/\/\*[\s\S]*?\*\//g, "").replace(/^\s*\/\/.*$/gm, "");

test("⚠️ יש שלט מרכזי — אחרת מפספסים אותו", () => {
  assert.match(barCode, /alert-unlock-overlay/, "שלט באמצע המסך");
  assert.match(barCode, /alert-unlock-card-btn/, "עם כפתור מפורש");
});

test("⚠️ והוא **אינו** נסוג — הפעלת הקול היא חובה", () => {
  // הנסיגה הייתה הדרך לעבוד בלי קול בלי לשים לב: 25 שניות, ואז פס דק
  // שאפשר לעבור לידו כל היום.
  assert.doesNotMatch(barCode, /RETREAT_MS|setRetreated|retreated/,
    "אסור שעון נסיגה — החלון נשאר עד שמפעילים");
  assert.doesNotMatch(barCode, /alert-unlock-bar/,
    "ואין פס דק לסגת אליו");
});

test("⚠️ גם מי שאישר בעבר מקבל את החלון", () => {
  // קודם נשמר "audio-opted-in" ומי שאישר פעם לא ראה כלום — והקול נשאר
  // חסום עד המגע הראשון. מדיניות ה-autoplay דורשת מגע **בכל טעינה**,
  // ולכן אין מה לזכור, וזיכרון כזה רק מסתיר שהקול כבוי.
  assert.doesNotMatch(barCode, /audio-opted-in|optedIn/,
    "אין זיכרון שמדלג על החלון");
});

test("⚠️ ושני הדברים שמקטינים את המחיר למסך קיר", () => {
  // רקע שקוף: רואים את המערכת עובדת מאחור, ולכן זה לא נקרא כמסך תקוע.
  assert.match(barCode, /alert-unlock-backdrop/, "רקע לחיץ ושקוף");
  // ההוראה שמייתרת את החלון לגמרי — הרשאת Sound לאתר.
  assert.match(barCode, /Sound/, "השלט אומר איך לא לראות אותו שוב");
});
...
// ============================================================
// ⚠️ הענבר אסור כאן — הוא צבע התחזוקה
// ============================================================
// שלט ההפעלה נצבע בענבר, שהוא הצבע של **מצב תחזוקה** בכל שאר המערכת
// (STATUS_COLORS.maintenance, פס הזמינות, ומקרא הגרפים). פס ענבר לרוחב
// המסך, או כפתור ענבר באמצעו, נקראים כמו "אתר בתחזוקה" — הודעה שגויה
// לגמרי, ובדיוק במקום שהעין סורקת ראשון.
//
// ⚠️ ונבדק כאיסור ולא כהמלצה: הכיוון הזה חוזר מעצמו, כי ענבר הוא הבחירה
// האינטואיטיבית ל"אזהרה". הבדיקה כאן היא מה שיעצור אותו.
const UNLOCK_CSS = fs.readFileSync(
  path.join(__dirname, "..", "..", "dashboard", "src", "components",
            "AlertBell", "AlertUnlockBar.css"), "utf8");

test("⚠️ שלט הקול משתמש בצבעי המותג ולא בענבר התחזוקה", () => {
  // הגוונים המדויקים שהיו כאן, ואלה שמופיעים ב-STATUS_COLORS.maintenance.
  assert.doesNotMatch(UNLOCK_CSS, /#b45309|#92400e|#fde68a|#f59e0b|#fbbf24/i,
    "ענבר = תחזוקה. השלט אינו הודעת תחזוקה.");
  assert.match(UNLOCK_CSS, /var\(--brand\)/, "הרקע בכחול המותג");
  assert.match(UNLOCK_CSS, /var\(--brand-lime\)/, "והליים מהלוגו כהדגשה");
});

test("⚠️ ונושא את הסימן — לא חלון גנרי", () => {
  // שלט שקופץ באמצע המסך על רקע מעומעם הוא הרגע שבו שואלים "מה זה".
  // סימן מוכר עונה על כך לפני שקוראים מילה.
  assert.match(barCode, /parkomat-logo\.png/, "הלוגו מוצג בשלט");
  assert.match(barCode, /SiteMonitor/, "עם שם המערכת, כמו במסך ההתחברות");
});
