...
// ============================================================
// ⚠️ **ההחלטה התהפכה: כן שלט מרכזי — אבל הוא חייב לסגת**
// ============================================================
// כאן ישבה בדיקה שאסרה overlay לחלוטין, בנימוק שחלון חוסם מסתיר את
// המערכת. הנימוק נכון — אבל הפס בראש המסך היה קל מדי לפספס, וזה נאמר
// מהשטח.
//
// הפתרון אינו לבחור אחד מהשניים אלא **לתזמן**: שלט מרכזי בהתחלה, ואחריו
// נסיגה לפס. ארבע הבדיקות הבאות מקבעות את **שני החצאים** — כי כל אחד
// מהם לבדו הוא בדיוק הבאג של השני.
...
test("⚠️ אבל הוא נסוג מעצמו — אחרת מסך קיר נשאר מוסתר", () => {
  // מסך קיר שמתאתחל בלילה יעלה, ישחזר את הסשן, וייתקע על השלט **בלי
  // להציג שום אתר** עד שמישהו יגיע בבוקר. קול חסר משאיר את המידע על
  // המסך; חלון שנשאר מוחק גם אותו.
  assert.match(barCode, /RETREAT_MS/, "חייב שעון נסיגה");
  assert.match(barCode, /setRetreated\(true\)/, "והוא חייב להחליף מצב באמת");
  assert.match(barCode, /alert-unlock-bar/, "ואחריו נשאר הפס — לא כלום");
});

test("⚠️ הנסיגה מתוזמנת, ולא מותנית בתנועת המשתמש", () => {
  // "נסוג רק אם אין תנועה" נשמע חכם ונכשל בדיוק במקרה שחשוב: במסך קיר
  // אין תנועה לעולם, ולכן התנאי היה מתקיים תמיד — או לעולם לא.
  assert.match(barCode, /setTimeout\(\s*\(\)\s*=>\s*setRetreated/,
    "שעון פשוט מתנהג זהה גם כשאיש אינו נוגע במסך");
  assert.doesNotMatch(barCode, /mousemove|pointermove/,
    "אסור לתלות את הנסיגה בתנועה");
});

test("⚠️ והשעון אינו רץ כשהקול כבר פעיל", () => {
  // בלעדיו הוא היה רץ ברקע תמיד, ומי שהשתיק והחזיר היה מקבל את הפס הדק
  // במקום השלט — הגרסה הפחות בולטת, בדיוק כשצריך אותה.
  assert.match(barCode, /state !== "locked" \|\| retreated/, "השעון מותנה במצב");
});
