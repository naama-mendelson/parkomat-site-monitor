...
// ⚠️ **הבדיקה הקודמת לא יכלה ליפול.** היא עברה רק מערכות `ready`/`operating`,
// ששתיהן טובות מ-`no_comm` בדירוג — כלומר היא הייתה ירוקה גם בלי שום חריג.
// `error` גרוע מ-`no_comm`, ולכן אתר מנותק שהפעימה האחרונה שלו ראתה מערכת
// בתקלה הוצג "תקלה" ברשימת האתרים ו"מנותק" במסך המפקח — שני מסכים, שני
// מצבים לאותו אתר באותו רגע.
test("אין תקשורת — גם פירוט ישן עם תקלה אינו גובר על הנתק", () => {
  assert.equal(displayStatusFor("no_comm", [{ state: "error" }, { state: "ready" }], FRESH, NOW), "no_comm");
});
